...
export default function WebsiteEditorPage() {
  ...
  const updateText = useCallback((text: string) => {
    ...
    const el = elRef.current;
    if (!el) return;
    // Only update if element has no child elements (pure text node)
  ...
  const onLoad = () => {
    ...
    doc.addEventListener('click',(e:MouseEvent)=>{ e.preventDefault();e.stopPropagation(); const t=e.target as HTMLElement; if(t&&t!==doc.body&&t!==doc.documentElement)selectEl(t); },true);
    doc.addEventListener('mouseover',(e:MouseEvent)=>{ const t=e.target as HTMLElement; if(t&&!t.hasAttribute('data-yp')&&t!==doc.body)t.style.outline='1px dashed rgba(245,158,11,0.4)'; },true);
  ...
  const applyWireframe = (doc:Document, on:boolean) => {
    ...
    if(on){ if(!s){s=doc.createElement('style');s.id='yp-wf';doc.head.appendChild(s);} s.textContent='*{background:transparent!important;background-image:none!important;color:#333!important;box-shadow:none!important;text-shadow:none!important} img,video{opacity:0.15!important} *{outline:1px solid rgba(0,0,0,0.06)!important}'; }
    else{ if(s)s.remove(); }
  };
  ...
  const saveAll = () => {
    ...
    setTimeout(()=>{if(elRef.current)selectEl(elRef.current);if(wireframe)applyWireframe(doc,true);},50);
  };

  const undoLast = () => { if(versions.length>0){setPageHtml(versions[versions.length-1].html);setSaved(false);} };
  const revertTo = (v:Version) => { setPageHtml(v.html); setShowVer(false); setSaved(false); setChangeCount(0); };
  const resetOriginal = () => { if(!confirm('Reset ALL changes?'))return; setPageHtml(originalHtmlRef.current); setSaved(false); setChangeCount(0); };
  ...
  // Uncontrolled color picker - uses ref to avoid re-render closing the popup
  const ColorPicker = ({label,prop}:{label:string;prop:string}) => {
    const ref = useRef<HTMLInputElement>(null);
    useEffect(() => { if(ref.current) ref.current.value = toHex(styles[prop]||''); }, [styles[prop]]);
    return (
      <div>
        <label className="text-[11px] text-gray-500 block mb-1">{label}</label>
        <div className="flex items-center gap-2">
          <input ref={ref} type="color" defaultValue={toHex(styles[prop]||'')} onChange={e=>apply(prop,e.target.value)} className="w-10 h-8 rounded border border-gray-300 cursor-pointer p-0.5" />
          <input value={styles[prop]||''} onChange={e=>apply(prop,e.target.value)} className="flex-1 h-8 rounded-md border border-gray-300 px-2 text-[12px] focus:border-amber-500 focus:outline-none" />
        </div>
      </div>
    );
  };
    ...
    <div className="h-screen flex flex-col bg-white">
      ...
      <div className="h-10 bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={resetOriginal} className="text-[11px] text-red-500 hover:text-red-700">Reset</button>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left toolbar - colorful boxed icons */}
        <div className="w-12 bg-gray-50 border-r border-gray-200 flex flex-col items-center py-3 gap-2 shrink-0">
          <button onClick={()=>setShowNav(!showNav)} className={`w-9 h-9 rounded-lg text-[9px] font-bold flex items-center justify-center transition-all ${showNav?'bg-amber-500 text-white shadow-md':'bg-white border border-gray-200 text-gray-600 hover:border-amber-400 hover:shadow'}`} title="Navigator">NAV</button>
          <button onClick={toggleWireframe} className={`w-9 h-9 rounded-lg text-[9px] font-bold flex items-center justify-center transition-all ${wireframe?'bg-gray-800 text-white shadow-md':'bg-white border border-gray-200 text-gray-600 hover:border-gray-400 hover:shadow'}`} title="Wireframe">WIR</button>
          ...
          <button onClick={()=>{}} className="w-9 h-9 rounded-lg text-[9px] font-bold flex items-center justify-center bg-white border border-gray-200 text-blue-600 hover:border-blue-400 hover:shadow hover:bg-blue-50 transition-all" title="Redo">RED</button>
        ...
        <div className="flex-1 overflow-hidden flex justify-center bg-gray-100">
          <div style={{width:viewport,maxWidth:'100%',height:'100%'}}>
            {pageHtml?<iframe ref={iframeRef} srcDoc={pageHtml} className="w-full h-full border-0 bg-white" onLoad={onLoad}/>:<div className="flex items-center justify-center h-full text-gray-400"><p>No page loaded. Go to Pages and click Edit.</p></div>}
          </div>
        </div>
        ...
        <div className="w-[280px] bg-white border-l border-gray-200 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {/* Content section - shows element text for editing */}
            <Sec id="content" title="Content">
              <div>
                <label className="text-[11px] text-gray-500 block mb-1">Element Text</label>
                <textarea value={textContent} onChange={e=>updateText(e.target.value)} placeholder="Select a text element to edit" className="w-full h-24 rounded-md border border-gray-300 px-3 py-2 text-[12px] text-gray-800 resize-none focus:border-amber-500 focus:outline-none" />
                <p className="text-[9px] text-gray-400 mt-1">Edit text here to update live on the page</p>
              </div>
            </Sec>
            <Sec id="text" title="Text">
              <Field label="Font Family" prop="fontFamily"/>
              <ColorPicker label="Color" prop="color"/>
              <div className="grid grid-cols-2 gap-2"><Field label="Font Size" prop="fontSize" unit="px"/><Sel label="Weight" prop="fontWeight" options={['','100','200','300','400','500','600','700','800','900']}/></div>
              <Field label="Line Height" prop="lineHeight"/>
              <div className="grid grid-cols-2 gap-2"><Sel label="Style" prop="fontStyle" options={['','normal','italic']}/><Sel label="Transform" prop="textTransform" options={['','none','uppercase','lowercase','capitalize']}/></div>
              <div className="grid grid-cols-2 gap-2"><Sel label="Decoration" prop="textDecoration" options={['','none','underline','line-through']}/><Sel label="Align" prop="textAlign" options={['','left','center','right','justify']}/></div>
              <Field label="Letter Spacing" prop="letterSpacing"/>
            </Sec>
            <Sec id="background" title="Background">
              <ColorPicker label="Background Color" prop="backgroundColor"/>
              <Field label="Image" prop="backgroundImage"/>
              <div className="grid grid-cols-2 gap-2"><Sel label="Size" prop="backgroundSize" options={['','auto','cover','contain']}/><Sel label="Repeat" prop="backgroundRepeat" options={['','repeat','no-repeat','repeat-x','repeat-y']}/></div>
            ...
            <Sec id="borders" title="Borders">
              <ColorPicker label="Border Color" prop="borderColor"/>
            ...
            <Sec id="radius" title="Border Radius">
              <div className="grid grid-cols-2 gap-2"><Field label="TL" prop="borderTopLeftRadius" unit="px"/><Field label="TR" prop="borderTopRightRadius" unit="px"/><Field label="BL" prop="borderBottomLeftRadius" unit="px"/><Field label="BR" prop="borderBottomRightRadius" unit="px"/></div>
            </Sec>
            ...
            <Sec id="measures" title="Measures">
              ...
              <Sel label="Display" prop="display" options={['','block','flex','grid','inline','inline-block','none']}/>
            ...
            <Sec id="extra" title="Extra (Flex/Grid)">
              <Sel label="Flex Dir" prop="flexDirection" options={['','row','column','row-reverse','column-reverse']}/>
              <Sel label="Align" prop="alignItems" options={['','stretch','center','flex-start','flex-end']}/>
              <Sel label="Justify" prop="justifyContent" options={['','flex-start','center','flex-end','space-between','space-around']}/>
