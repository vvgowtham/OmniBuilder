  ...
  const sendPrompt = async () => {
    ...
    setTimeout(() => {
      ...
      if (prompt.includes('hero') || prompt.includes('landing')) {
        response = 'Generated a hero section with headline, subtitle, and CTA button. You can add it to the visual builder.';
        ...
        response = 'Created a contact form with name, email, and message fields.';
        ...
        response = 'Generated a 3-tier pricing table.';
        ...
        response = 'I can generate hero sections, contact forms, pricing tables, navbars, footers, and more. Try asking for something specific!';
      }
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {messages.length === 0 && (
              <div className="text-center text-gray-400 py-12">
                <p className="text-lg mb-2">AI</p>
                <p className="text-sm">Ask me to generate page sections, components, or entire pages.</p>
                <p className="text-xs mt-2">Try: &quot;Generate a hero section for a SaaS product&quot;</p>
              </div>
            )}
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm ${m.role === 'user' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-800'}`}>
                  {m.content}
                </div>
              </div>
            ))}
            {loading && (
              <div className="flex justify-start">
                <div className="bg-gray-100 px-4 py-2.5 rounded-2xl text-sm text-gray-500">Generating...</div>
              </div>
            )}
          </div>
          <div className="p-4 border-t border-gray-100">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 h-10 rounded-lg border border-gray-200 px-3 text-sm focus:border-purple-500 focus:outline-none"
              />
              <button
                onClick={sendPrompt}
                disabled={loading || !input.trim()}
                className="h-10 px-4 rounded-lg bg-purple-600 text-white text-sm font-semibold disabled:opacity-50"
              >
                Send
              </button>
            </div>
