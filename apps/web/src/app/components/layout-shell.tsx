...
const NAV = [
  ...
  {href:'/style-editor',label:'Style Editor',icon:'YP'},
...
export default function LayoutShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-gray-50">
      <aside className={`${collapsed ? 'w-[52px]' : 'w-[180px]'} bg-gradient-to-b from-[#110c22] to-[#0d0919] border-r border-white/5 flex flex-col shrink-0 transition-all duration-200`}>
        ...
        <nav className="flex-1 overflow-y-auto p-1 space-y-0.5">
          {NAV.map(item => (
            <Link key={item.href} href={item.href} className={`flex items-center gap-2 px-2 py-1.5 rounded-lg text-[10px] font-medium transition-all ${pathname === item.href ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-sm' : 'text-white/50 hover:bg-white/5 hover:text-white/80'}`}>
        ...
        <div className="p-1.5 border-t border-white/5">
          <button onClick={() => setCollapsed(!collapsed)} className="w-full text-[9px] text-purple-300/50 hover:text-white py-1 rounded hover:bg-white/5 transition-all">{collapsed ? '>' : 'Collapse'}</button>
          <button onClick={() => { localStorage.clear(); router.push('/login'); }} className="w-full text-[9px] text-purple-300/50 hover:text-white py-1 rounded hover:bg-white/5">{collapsed ? '' : 'Logout'}</button>
        </div>
      </aside>
